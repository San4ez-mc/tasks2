...
<div class="simple-form-page">
    <div class="simple-form-card">
        ...
        <form method="post" action="/projects/edit/<?php echo (int) ($project['id'] ?? 0); ?>">
            <div class="form-group">
                <label for="name">Назва проекту *</label>
                <input type="text" id="name" name="name" required
                    value="<?php echo htmlspecialchars((string) ($project['name'] ?? '')); ?>">
            </div>

            <div class="form-group">
                <label for="description">Опис</label>
                <textarea id="description"
                    name="description"><?php echo htmlspecialchars((string) ($project['description'] ?? '')); ?></textarea>
            </div>
                ...
                <?php else: ?>
                    <div class="members-list">
                        <?php foreach ($employees as $emp): ?>
                            ...
                            if ($ename === '')
                                $ename = (string) ($emp['email'] ?? '#' . $uid);
                            $checked = in_array($uid, $member_ids, true);
                            ?>
                            <label class="member-item">
                                <input type="checkbox" name="member_ids[]" value="<?php echo $uid; ?>" <?php echo $checked ? 'checked' : ''; ?>>
